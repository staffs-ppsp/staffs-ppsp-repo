...
public class Seats {
	...
	private int intShowingID; // Number to represent the Showing ID that the
								// seat belongs to
	private int intCustomerID; // Number to represent the Customer ID that the
								// seat belongs to
	...
	public Seats(Booking _objBooking) {
		this.strScreen = this.getStrScreen();
		this.intCustomerID = _objBooking.getIntCustomerID();
	 ...
	 *             booked
	 */
	public boolean addSeat(Seats _objSeats) {
		try {

			// if (!getCurrentDateTime().equalsIgnoreCase(
			// objBooking.getShowingTime())) {
			for (int i = 0; i < _objSeats.getIntNumSeats(); i++) {
				if (arrSeats[_objSeats.objRow.getRowNum()][_objSeats.objColumn
					...
					System.out
							.println(arrSeats[_objSeats.objRow.getRowNum()][_objSeats.objColumn
									.getColNum() + i]);
					throw new Exception();
				}
			}
	...
	private void setStrScreen(String strScreen) {
	...
	public boolean setObjRow(Row _objRow) {
		this.objRow = _objRow;
		return true;
	}
	...
	public boolean setObjColumn(Column _objColumn) {
		this.objColumn = _objColumn;
		return true;
	}
